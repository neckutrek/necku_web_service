...
void cerr_if_failed(const error_code& ec, const std::string& msg)
{
  if (ec.failed())
  {
    std::string errmsg = msg + ". " + ec.message() + ". " + ec.category().name();
...
void connect() {
  ...
  try {
    ...
    ws_socket.set_option(websocket::stream_base::decorator(
        [&api_key](websocket::request_type &req) -> void {
    ...
    while (n < 0) {
      ...
      //std::for_each(buffer.data().begin(), buffer.data().end(), [](auto el) -> void {
    ...
    //ws_socket.next_layer().shutdown(ec);
    //::throw_if_failed(ec, "Failed to shutdown TLS");

    int i;
    std::cout << "Go now? " << std::endl;
    std::cin >> i;
    ws_socket.close(websocket::close_code::none, ec);
    ::cerr_if_failed(ec, "Failed to close websocket");
    //::throw_if_failed(ec, "Failed to close websocket");

    //boost::beast::multi_buffer buffer;
    //ws_socket.read(buffer, ec);
    //::throw_if_failed(ec, "Failed to read incoming close frame");
    //std::cout << "Read from websocket" << std::endl;
    //std::for_each(buffer.data().begin(), buffer.data().end(), [](auto el) -> void {
    //  std::cout.write(static_cast<const char *>(el.data()), el.size());
    //});
